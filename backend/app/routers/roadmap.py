...
from app.schemas.progress import ProgressUpdateRequest, ProgressResponse
from app.schemas.roadmap import RoadmapSeed, RoadmapPhaseFullResponse
...
@router.get(
    "/{profession_id}",
    response_model=list[RoadmapPhaseFullResponse],
    summary="Дорожная карта для профессии",
)
async def get_roadmap(
    ...
    # 1. Загружаем roadmap с топиками
    result = await db.execute(
        select(RoadmapPhase)
    ...
    # 2. Загружаем прогресс (только если пользователь авторизован)
    ...
    # 3. Вешаем атрибуты прогресса на topic-объекты.
    for phase in phases:
        for topic in phase.topics:
            progress = progress_map.get(topic.id)
            topic.is_completed = progress.is_completed if progress else None
            topic.completed_at = progress.completed_at if progress else None

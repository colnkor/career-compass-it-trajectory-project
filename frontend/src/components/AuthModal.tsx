  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-md bg-[rgba(15,17,25,0.95)] border border-border-soft rounded-[23px] p-8 flex flex-col gap-0 shadow-glass backdrop-blur-xl">
        ...
        {/* API error */}
        {apiError && (
          <div className="mt-4 px-4 py-3 rounded-xl bg-danger/10 border border-danger/25 text-danger text-sm">
            {apiError}
          </div>
        )}
        ...
        {showOAuth && (
          <>
            <div className="flex items-center gap-3 my-5">
              ...
              <span className="text-xs text-muted">Или через</span>
              <div className="flex-1 h-px bg-border-soft" />
            </div>
        ...
        <p className="text-center text-sm text-muted mt-6">
          ...
          <button
            ...
            className="text-accent-light hover:text-text transition-colors font-medium cursor-pointer"
          >
...
function OAuthButton({ children }: { children: React.ReactNode }) {
  return (
    <button className="w-12 h-12 rounded-full bg-card border border-border-soft hover:border-accent-light/30 flex items-center justify-center transition-all duration-200 cursor-pointer">
      {children}
    </button>
  );
}